...
#
# flags = tf.app.flags
# # default setting of 'mode' is 'train'
# flags.DEFINE_string('mode', 'train', "'pretrain', 'train' or 'eval'")
# flags.DEFINE_string('model_save_path', 'model_resize', "directory for saving the model")
# flags.DEFINE_string('sample_save_path', 'sample_resize', "directory for saving the sampled images")
# FLAGS = flags.FLAGS
#
#
# def main(_):
...
#     #                svhn_dir='svhn', mnist_dir='mnist2', model_save_path=FLAGS.model_save_path, sample_save_path=FLAGS.sample_save_path)
#     solver = Solver(model, batch_size=100, pretrain_iter=5000, train_iter=15000, sample_iter=100,
#                     svhn_dir='svhn', mnist_dir='mnist2', model_save_path=FLAGS.model_save_path,
#                     sample_save_path=FLAGS.sample_save_path)
#     #
#     ######################################################################################################################################
#     # create directories if not exist(just directories)
...
#     #  tf.app.run()
#     #########################################################################################################
#     model = DTN(mode=FLAGS.mode, learning_rate=0.03)  # 0.001, droup out 없음
#     solver = Solver(model, batch_size=100, pretrain_iter=2000, train_iter=50000, sample_iter=100,
#                     svhn_dir='svhn', mnist_dir='mnist2', model_save_path=FLAGS.model_save_path,
#                     sample_save_path=FLAGS.sample_save_path)
#     if not tf.gfile.Exists(FLAGS.model_save_path):
#         tf.gfile.MakeDirs(FLAGS.model_save_path)
...
#                         net = slim.conv2d(images, 16, [11, 11], scope='conv1', padding='VALID',
#                                           activation_fn=tf.nn.relu)  # (batch_size, 142, 142, 32)
...
#                         net = slim.conv2d(net, 32, [9, 9], scope='conv2', padding='VALID',
#                                           activation_fn=tf.nn.relu)  # (batch_size, 63, 63, 16)
...
#                         net = slim.conv2d(net, 64, [9, 9], scope='conv3', padding='VALID',
#                                           activation_fn=tf.nn.relu)  # (batch_size, 24, 24, 16)
#                         print("conv3" + str(np.shape(net)))
#                         # net = slim.batch_norm(net, scope='bn3')
#                         # print("batch3" + str(np.shape(net)))
#                         net = slim.conv2d(net, 100, [7, 7], scope='conv4', padding='VALID',
#                                           activation_fn=tf.nn.relu)  # (batch_size, 18, 18, 16)
...
#
#                         # Block 1
#                         net = slim.conv2d_transpose(inputs, 512, [5, 5], padding='VALID',
...
#                         # Block 2
#                         net = slim.conv2d_transpose(net, 128, [3, 3], padding='VALID',
#                                                     scope='conv_transpose2')  # (batch_size, 8, 8, 256)
...
#                                                     scope='conv_transpose3')  # (batch_size, 16, 16, 128)
...
#
#                         # Block 4
#                         net = slim.conv2d_transpose(net, 16, [2, 2], padding='VALID', stride=1,
...
#
#                         # Block 1
#                         net = slim.conv2d(images, 3, [3, 3], activation_fn=tf.nn.relu,
#                                           scope='conv1')  # (batch_size, 76, 76, 128)
#                         net = slim.batch_norm(net, scope='bn1')
...
#
#                 self.D_loss_1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=self.D_logits_1,
#                                                                                        labels=np.array(
...
#                      in range(self.batch_size)]))) \
#                                  + tf.reduce_mean(
#                     tf.nn.softmax_cross_entropy_with_logits(logits=self.D_logits_2, labels=
#                     np.array(
